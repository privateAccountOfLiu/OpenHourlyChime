...
class SettingsFragment : PreferenceFragmentCompat() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        val noticePreference = findPreference<Preference>("notifications_enabled")
        ...
        val languagePreference = findPreference<Preference>("language_preference")

        noticePreference?.setOnPreferenceChangeListener { _, new ->
            listener?.onPreferenceChanged("notifications_enabled", new)
            context?.let { ToastUtil.showToast(it, getString(R.string.toast_1)) }
            true
        }

        testBroadCastPreference?.setOnPreferenceClickListener {
            listener?.onPreferenceClicked("test_broad_cast_preference")
            Toast.makeText(context, getString(R.string.toast_2), Toast.LENGTH_SHORT).show()
            true
        }
        ...
        languagePreference?.setOnPreferenceChangeListener { _, new ->
            listener?.onPreferenceChanged("language_preference", new)
            context?.let { ToastUtil.showToast(it, getString(R.string.toast_1)) }
            true
        }
    }
